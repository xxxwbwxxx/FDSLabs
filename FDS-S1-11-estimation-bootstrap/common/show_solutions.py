...
def parse_txt_solution(solution_id):
    try:
        solution_id = solution_id.strip("\n").strip()
        out_buffer = str()
        with open(os.path.join('common', 'hints_solutions', "solutions.txt"), 'r') as j:
            start_read = False
...
def parse_json(question):
    with open(os.path.join('common', 'hints_solutions', "hints_solutions.json"), 'r') as j:
...
def show(question):
    ...
    question_ans = parse_json(str(question))
    ...
    # Set the data
    solution = parse_txt_solution(question_ans['solution'])
